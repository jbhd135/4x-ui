...
function formatClientExpiryTime(expiryTime) {
    ...
    return `过期日期:${expiry.getFullYear()}年${expiry.getMonth() + 1}月${expiry.getDate()}日`;
}

function formatClientIdentifierSuffix(email) {
    const identifier = `${email || ''}`.trim();
    if (!identifier) return '';

    const datedIdentifier = identifier.match(/^(?:\d{4}|\d{6}|\d{8})-(.+)$/);
    return datedIdentifier ? datedIdentifier[1] : identifier;
}
...
class Inbound extends XrayCommonClass {
    ...
    genAllLinks(remark = '', remarkModel = '-ieo', client) {
        ...
        const clientExpiryRemark = formatClientExpiryTime(client?.expiryTime);
        const clientIdentifierSuffix = formatClientIdentifierSuffix(email);
        const visibleRemark = clientExpiryRemark ?
            [clientExpiryRemark, clientIdentifierSuffix].filter(Boolean).join(separationChar) :
            formatClientExpiryRemark(remark);
        const isExpiryRemark = clientExpiryRemark.length > 0 || visibleRemark !== `${remark || ''}`.trim();
        let orders = {
            'i': visibleRemark,
...
Inbound.ClientBase = class extends XrayCommonClass {
    constructor(
        ...
        this.socksProxyEnabled = Boolean(socksProxyEnabled);
    }

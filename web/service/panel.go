...
// GetUpdateInfo checks GitHub for the latest GG Panel release.
...
// StartUpdate starts the GG Panel updater outside of the current web request.
func (s *PanelService) StartUpdate() error {
	...
	updateScript := fmt.Sprintf("set -o pipefail; %s -fLs https://raw.githubusercontent.com/jbhd135/gg-panel/main/update.sh | %s", shellQuote(curl), shellQuote(bash))
...
func fetchLatestPanelVersion() (string, error) {
	...
	resp, err := client.Get("https://api.github.com/repos/jbhd135/gg-panel/releases/latest")
	if err != nil {
		return "", err
	}

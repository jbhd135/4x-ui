...
func applyInboundSocksProxy(xrayConfig *xray.Config, inbound *model.Inbound) error {
	...
	clientEmails, err := socksProxyClientEmails(inbound)
	if err != nil {
		return err
	}
	...
	routing["rules"] = prependAfterAPIRule(rules, buildSocksProxyRoutingRules(inbound.Tag, outboundTag, clientEmails)...)
...
func socksProxyClientEmails(inbound *model.Inbound) ([]string, error) {
	...
	for _, client := range settings.Clients {
		...
		if !client.SocksProxyEnabled || email == "" {
			continue
		}
...
func buildSocksProxyRoutingRules(inboundTag string, socksOutboundTag string, clientEmails []string) []map[string]any {

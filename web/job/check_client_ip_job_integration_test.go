...
func TestUpdateInboundClientIps_OldLogEntryDoesNotCountAsLive(t *testing.T) {
	setupIntegrationDB(t)

	const email = "active-window-regression"
	seedInboundWithClient(t, "inbound-active-window", email, 1)

	now := time.Now().Unix()
	row := seedClientIps(t, email, nil)
	observed := []IPWithTimestamp{
		{IP: "10.2.0.1", Timestamp: now - int64(clientIPActiveWindow/time.Second) - 5},
		{IP: "192.0.2.10", Timestamp: now},
	}

	j := NewCheckClientIpJob()
	shouldCleanLog := j.updateInboundClientIps(row, email, observed)

	if shouldCleanLog {
		t.Fatal("an old log entry must not cause a current device-limit ban")
	}
	if len(j.disAllowedIps) != 0 {
		t.Fatalf("no IP should be banned, got %v", j.disAllowedIps)
	}
	persisted := ipSet(readClientIps(t, email))
	for _, want := range []string{"10.2.0.1", "192.0.2.10"} {
		if _, ok := persisted[want]; !ok {
			t.Errorf("expected %s to remain in recent history; got %v", want, persisted)
		}
	}
}

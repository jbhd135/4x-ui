...
func (a *InboundController) initRouter(g *gin.RouterGroup) {
	...
	g.POST("/:id/clientDailyTrafficLimit/:email", a.updateClientDailyTrafficLimit)
...
func (a *InboundController) updateClientDailyTrafficLimit(c *gin.Context) {
	id, err := strconv.Atoi(c.Param("id"))
	if err != nil {
		jsonMsg(c, I18nWeb(c, "pages.inbounds.toasts.inboundUpdateSuccess"), err)
		return
	...
	traffic, needRestart, err := a.inboundService.UpdateClientDailyTrafficLimit(
		id,
		c.Param("email"),
		request.DailyTrafficLimit,
	)
	if err != nil {
		jsonMsg(c, I18nWeb(c, "somethingWentWrong"), err)
		return
	}
	jsonMsgObj(c, I18nWeb(c, "pages.inbounds.toasts.inboundUpdateSuccess"), traffic, nil)
	if needRestart {
		a.xrayService.SetToNeedRestart()
	}
